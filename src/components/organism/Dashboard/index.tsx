...
export function DashboardCard() {
  ...
  devices.forEach((device) => {
    ...
    // Tentukan status akhir device
    if (isStatusActive && !isTimedOut) {
      onlineCount++;
    } else {
      offlineCount++;
    }
  ...
  return (
    <Card variant="brutal" animate={false}>
          ...
          <div className="flex w-full justify-between items-center">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200">
                <Radio
                  size={14}
                  className={onlineCount > 0 ? "animate-pulse" : ""}
                />
              </div>
            ...
            {/* Indikator Status Minimalis Masing-Masing Perangkat */}
            <div className="flex gap-3 items-center text-[10px] font-mono font-black tracking-wider">
              <div className="flex items-center gap-1.5">
                <span
                  className={`w-2 h-2 border rounded-full ${onlineCount > 0 ? "bg-green-500 border-green-500 dark:bg-green-100 dark:border-zinc-100 animate-pulse" : "bg-green-100 border-green-300"}`}
                />
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className={`w-2 h-2 border rounded-full ${offlineCount > 0 ? "bg-red-400 border-red-400" : "bg-red-200 border-red-200"}`}
                />
      ...
      <CardContent className="pt-2 pb-4">
        <p className="font-black text-3xl tracking-tight text-zinc-900 dark:text-zinc-50">
          {onlineCount}
